...
class Game():
    ...
    def __init__(self,emotes):
        ...
        self.botrect=['','','']
        for i in range(0,3):
            self.top[i],self.toprect[i]=load_image(emotes[(self.current[i]+1)%7])
            self.mid[i],self.midrect[i]=load_image(emotes[self.current[i]])
            self.bot[i],self.botrect[i]=load_image(emotes[(self.current[i]-1)%7])
        ...
        for i in range(0,3):
            if self.spin[i]:
                if self.y[i]>28:
                    ...
                    self.current[i]+=1
                    self.top[i],self.toprect[i]=load_image(emotes[(self.current[i]+1)%7])
                    self.mid[i],self.midrect[i]=load_image(emotes[self.current[i]%7])
                    self.bot[i],self.botrect[i]=load_image(emotes[(self.current[i]-1)%7])
                else:
                    self.y[i]+=self.wheelspeed[i]
                if (self.current[i]%7)==0:
                    if self.y[i]==0:
                        self.spincount[i]=self.current[i]/7
